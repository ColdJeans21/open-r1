...
# def accuracy_reward(completions: list[list[dict[str, str]]], answer: list[str], **kwargs) -> list[float]:
#     """Reward function that checks if the completion matches the pre-extracted answer column."""
#     contents = [completion[0]["content"] for completion in completions]
#     rewards = []
    
#     # 注意这里：zip 遍历的是 contents 和直接传进来的 answer 列
#     for content, ans in zip(contents, answer):
#         # 1. 解析标准答案 (因为 ans 已经是提取好的简短答案，直接 parse 即可)
#         gold_parsed = parse(
#             ans,
#             extraction_mode="first_match",
#         )
...
#             # 3. 验证两者是否在数学上等价
#             try:
#                 # 建议：将原代码的 None 替换为 0.0，因为最新版本的 trl 遇到 None 可能会导致 loss 计算崩溃
#                 reward = float(verify(gold_parsed, answer_parsed))
#             except Exception as e:
#                 print(f"verify failed: {e}, answer: {answer_parsed}, gold: {gold_parsed}")
#                 reward = 0.0 
#         else:
#             # 如果极端情况下标准答案没解析出来，给 0.0 分
#             reward = 0.0
...
#                     ),
#                     boxed_match_priority=0,
#                     try_extract_without_anchor=True,  # 🌟 最关键的一行：允许裸数据解析
#                 )
#             ],
#             extraction_mode="first_match",
#         )
...
#                         ),
#                         boxed_match_priority=0,
#                         try_extract_without_anchor=False, # 这里依然保持 False，强迫模型输出标准的 boxed
#                     )
#                 ],
#                 extraction_mode="first_match",
#             )
